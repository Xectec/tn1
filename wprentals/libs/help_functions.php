...
function wpestate_generate_trip_details($property_id,$invoice_id,$bookid,$mode=''){
    ...
    print '<div class="trip_details_wrapper"><div class="trip_details_container" style="'.esc_attr($mode_wrapper_trip_details_container).'" >';
        ...
        $from_weekday = date_i18n(__('l', 'wprentals'), $booking_from_date_unix);
        $to_weekday = date_i18n(__('l', 'wprentals'), $booking_to_date_unix);
        ...
        if($do_we_show=='yes'):
            ...
            if($smoking_allowed!=''){
                ...
                if($smoking_allowed=='yes'){
                    print esc_html('yes','wprentals');
                }else{
                    print esc_html('no','wprentals');
                }
            ...
            $checkin_message   = esc_html(get_post_meta($property_id, 'checkin_message', true));

            if(trim($checkin_message)!=''){
                print '<h6 style="'.esc_attr($mode_wrapper_h6).'" >'.esc_html__('Owner notes','wprentals').'</h6>';
                print esc_html($checkin_message).'</br>';
            }      
...
if (!function_exists('wpestate_show_booking_button')):
    ...
    function wpestate_show_booking_button($post_id) {
       ...
       if( wprentals_get_option('wp_estate_show_booking_terms')=='yes'){
            print '<div class="wprentals_booking_check_wrapper">
                ...
                <label for="wprentals_booking_check_terms"> <a target="_blank" href="'.esc_url(wprentals_get_option('wp_estate_show_booking_terms_link')).'">'.wprentals_get_option('wp_estate_show_booking_terms_label').'</a></label>
                </div>
            ';
        }
...
if (!function_exists('wpestate_print_property_unit_slider')):
    ...
    function wpestate_print_property_unit_slider($post_id, $wpestate_property_unit_slider, $wpestate_listing_type, $wpestate_currency, $wpestate_where_currency, $link, $return_type = '') {
        ...
        if($wpestate_listing_type==1 || $wpestate_listing_type==3 || $wpestate_listing_type==4 ){
        ...
        print '<div class="" itemprop="offers" itemscope itemtype="http://schema.org/Offer">    
        <link itemprop="url" href="'. esc_url($link).'"/>   
        <meta itemprop="priceCurrency" content="'. esc_html($currency_code).'" />';
            print '<div class="price_unit">';
            print '<span itemprop="price" content="'.floatval($price).'">';
...
if (!function_exists('wpestate_share_unit_desing')):
    ...
    function wpestate_share_unit_desing($prop_id, $is_single = 1) {
        ...
        <div class="prop_social">
            ...
            <a href="<?php print esc_html($protocol);?>://twitter.com/intent/tweet?text=<?php echo esc_html($twiter_status); ?>" class="share_tweet" target="_blank" rel ="noreferrer" ><i class="fa-brands fa-x-twitter"></i></a>
...
if (!function_exists('wpestate_delete_cache')):
    ...
    function wpestate_delete_cache() {
        ...
        delete_transient('envato_purchase_code_12921802_demos' );
    }
...
if (!function_exists('wpestate_calculate_distance_geo')):
    ...
    function wpestate_calculate_distance_geo($lat, $long, $start_lat, $start_long, $yelp_dist_measure) {
        $lat        = floatval($lat);
        $long       = floatval($long);
        $start_long = floatval($start_long);
...
if (!function_exists('wpestate_show_mandatory_fields')):
    ...
    function wpestate_show_mandatory_fields() {
        ...
        if (is_array($mandatory_fields) && !empty($mandatory_fields)) {
            ...
            $wp_estate_category_main_label          =    wprentals_get_option('wp_estate_category_main', '');
            $wp_estate_category_second_label        =    wprentals_get_option('wp_estate_category_second', '');
            
            if( $wp_estate_category_main_label!='' ){
                $all_mandatory_fields['prop_category_submit']=  $wp_estate_category_main_label;
            }

            if( $wp_estate_category_second_label!='' ){
                $all_mandatory_fields['prop_action_category_submit']=  $wp_estate_category_second_label;
            }
