    ...
    function wpestate_send_valid_sms()
    {
        ...
        if (trim($user_mobile)!='') {
            ...
            if(class_exists('WpestateEmail') ){
                    ...
                    $WpestateEmail->wpestate_select_sms_type($user_mobile, 'validation', $arguments, $user_email, $userID);
            }
...
function wpestate_show_contact_form_simple($agent_id, $post_id)
{
    ...
    print'
        ...
        <div class=" has_calendar calendar_icon first_calendar">
            <input type="text" id="booking_from_date" size="40" name="booking_from_date" class="form-control" placeholder="'.esc_html__('Check-In', 'wprentals').'"
            value="';
        ...
        <div class=" has_calendar calendar_icon">
            <input type="text" id="booking_to_date" size="40" name="booking_to_date" class="form-control" placeholder="'.esc_html__('Check-Out', 'wprentals').'" 
            value="';
        ...
        if( wprentals_get_option('wp_estate_custom_guest_control','') == 'yes'){  
            print '<div class=" has_calendar guest_icon " >';
                echo wpestate_show_advanced_guest_form(esc_html__('Guests','wprentals'), '',$post_id); 
            print '</div>';
        }else{
            print '<select id="booking_guest_no"  name="booking_guest_no"  class="cd-select form-control booking_guest_no_select" >
                <option value="1">1 '.esc_html__('Guest   ', 'wprentals').'</option>';
                    for ($i = 2; $i <=    intval(wprentals_get_option('wp_estate_guest_dropdown_no', '')); $i++) {
                        ...
                        if ( isset($_GET['guest_no_prop']) && intval($_GET['guest_no_prop']) ==$i  ) {
                            print ' selected ';
                        }
                        
                        print '>'.$i.' '.esc_html__('Guests', 'wprentals').'</option>';
                    }
            print'</select>';
        }
