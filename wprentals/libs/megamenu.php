   ...
   // var $tree_type = array( 'post_type', 'taxonomy', 'custom' );
    ...
    function start_el(&$output, $item, $depth = 0, $args = array(), $current_object_id = 0) {
        ...
        if ( ! empty( $item->_invalid ) ) {
            ...
            $title = sprintf( __( '%s (Invalid)', "wprentals"), $item->title );
        } elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
            $classes[] = 'pending';
            $title = sprintf( __('%s (Pending)', "wprentals"), $item->title );
        }
                ...
                <dt class="menu-item-handle">
                    ...
                    <span class="item-controls">
                        ...
                        <span class="item-order hide-if-js">
                            ...
                            ?>" class="item-move-up"><abbr title="<?php esc_attr_e('Move up', "wprentals"); ?>">&#8593;</abbr></a>
                            |
                            <a href="<?php
                                echo wp_nonce_url(
                                    add_query_arg(
                            ...
                            ?>" class="item-move-down"><abbr title="<?php esc_attr_e('Move down',"wprentals"); ?>">&#8595;</abbr></a>
                        </span>
                        <a class="item-edit" id="edit-<?php echo esc_html($item_id); ?>" title="<?php esc_attr_e('Edit Menu Item', "wprentals"); ?>" href="<?php
                        ...
                        ?>"><?php _e( 'Edit Menu Item', "wprentals"); ?></a>
                    </span>
                </dt>
            </dl>
            ...
            <div class="menu-item-settings" id="menu-item-settings-<?php echo esc_html($item_id); ?>">
                ...
                <p class="description description-thin">
                    <label for="edit-menu-item-title-<?php echo esc_html($item_id); ?>">
                        <?php _e( 'Navigation Label', "wprentals" ); ?><br />
                ...
                <p class="description description-thin">
                    <label for="edit-menu-item-attr-title-<?php echo esc_html($item_id); ?>">
                        <?php _e( 'Title Attribute', "wprentals" ); ?><br />
                ...
                <p class="field-link-target description">
                    <label for="edit-menu-item-target-<?php echo esc_html($item_id); ?>">
                        ...
                        <?php _e( 'Open link in a new window/tab', "wprentals" ); ?>
                    </label>
                </p>
                <p class="field-css-classes description description-thin">
                    <label for="edit-menu-item-classes-<?php echo esc_html($item_id); ?>">
                        <?php _e( 'CSS Classes (optional)', "wprentals" ); ?><br />
                ...
                <p class="field-xfn description description-thin">
                    <label for="edit-menu-item-xfn-<?php echo esc_html($item_id); ?>">
                        <?php _e( 'Link Relationship (XFN)', "wprentals"  ); ?><br />
                ...
                <p class="field-description description description-wide">
                    <label for="edit-menu-item-description-<?php echo esc_html($item_id); ?>">
                        <?php _e( 'Description', "wprentals" ); ?><br />
                        ...
                        <span class="description"><?php _e('The description will be displayed in the menu if the current theme supports it.', "wprentals"); ?></span>
                    </label>
                </p>
                ...
                <label for="edit-menu-item-target-<?php echo esc_html($item_id); ?>">
                        <strong><?php _e( 'Menu Item Icon(ex: fa fa-comment)', "wprentals" ); ?></strong><br />
                ...
                <p class="description description-wide">
                <label for="edit-menu-item-target-<?php echo esc_html($item_id); ?>">
                        <strong><?php _e( 'Menu Item Label(ex: "new")', "wprentals" ); ?></strong><br />
                ...
                <p class="field-megamenu-checkbox">
                    ...
                    <label for="edit-menu-item-megamenu-<?php echo esc_html($item_id); ?>">
                        ...
                        <strong><em><?php _e( 'Set as Mega Menu?', "wprentals" ); ?></em></strong>
                    </label>
                </p>
                 ...
                 <p class="field-megamenu-widgets description description-wide">
                    <label for="edit-menu-item-megamenu-widgetarea-<?php echo esc_html($item_id); ?>">
                        <?php _e( 'Mega Menu Widget Area', 'wprentals' ); ?>
                        <select id="edit-menu-item-megamenu-widgetarea-<?php echo esc_html($item_id); ?>" class="widefat code edit-menu-item-megamenu-widgetarea" name="menu-item-megamenu-widgetarea[<?php echo esc_html($item_id); ?>]">
                            <option value="0"><?php _e( 'Select Widget Area', 'wprentals' ); ?></option>
                            <?php
                ...
                <a href="#" id="wpestate-media-upload-<?php echo esc_html($item_id); ?>" class="load_back_menu  "><?php _e( 'Upload Background Image | ', 'wprentals' ); ?></a>
              
                <a href="#" id="wpestate-media-remove-<?php echo esc_html($item_id); ?>" class="remove-megamenu-background" style="<?php echo ( trim( $item->megamenu_background ) ) ? 'display: inline;' : '';?>"><?php _e('Remove Image','wprentals');?></a>
                ...
                <p class="field-megamenu-styles description description-wide">
                    <label for="edit-menu-item-megamenu-styles-<?php echo esc_html($item_id); ?>">
                        <?php _e( 'Container Styles( *set custom styles for mega menu container only:.  Ex: background position, background repeat )', "wprentals" ); ?><br />
                ...
                <p class="field-megamenu-checkbox-border">
                    ...
                    <label for="menu-item-megamenu-border-<?php echo esc_html($item_id); ?>">
                        ...
                        <strong><em><?php _e( 'Draw border right on mega menu column?', "wprentals" ); ?></em></strong>
                    </label>
                </p>
                ...
                <div class="menu-item-actions description-wide submitbox">
                    <?php if( 'custom' != $item->type && $original_title !== false ) : ?>
                        <p class="link-to-original">
                            <?php printf( __('Original: %s', "wprentals"), '<a href="' . esc_url( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
                        </p>
                    <?php endif; ?>
                    ...
                    ); ?>"><?php _e('Remove', "wprentals" ); ?></a> <span class="meta-sep"> | </span> <a class="item-cancel submitcancel" id="cancel-<?php echo esc_html($item_id); ?>" href="<?php echo esc_url( add_query_arg( array('edit-menu-item' => $item_id, 'cancel' => time()), remove_query_arg( $removed_args, esc_url( admin_url( 'nav-menus.php' ) ) ) ) );
                        ?>#menu-item-settings-<?php echo esc_html($item_id); ?>"><?php _e('Cancel', "wprentals"); ?></a>
                </div>
        ...
        function end_lvl(&$output, $depth = 0, $args = array()) {
            ...
            if($depth === 0) 
            {
                if($this->active_megamenu)
                {
                    ...
                    {
                        $output = str_replace("{current_row_".$row."}", "wpestate_megamenu_col_".$columns, $output);
                    }
