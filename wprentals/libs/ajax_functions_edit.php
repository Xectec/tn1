    ...
    function wpestate_ajax_add_booking(){
        ...
        if ( $owner_id == $userID ) {
            ...
            if($instant_booking==1){
                ...
                wpestate_send_booking_email('newbook',$receiver_email,$new_book_details);
            }else{
                //normal]
                wpestate_add_to_inbox($userID,$userID,$to, $subject,$description,"external_book_req");
                wpestate_send_booking_email('newbook',$receiver_email,$new_book_details);
            ...
            if( !is_numeric($_POST['listing_edit'] ) ){
                ...
                if( $current_user->ID != $the_post->post_author ) {
                    ...
                    while($i< count($beds_options)){
                        ...
                        $beds_options_array[trim(wpestate_convert_cyrilic($bed_types_array[$index_bed_options])) ][]= $beds_options[$i];
                        $i++;
                    ...
                    $other_rules            =   sanitize_textarea_field($_POST['other_rules']);
                    $smoking_allowed        =   sanitize_text_field($_POST['smoking_allowed']);
                    $pets_allowed           =   sanitize_text_field($_POST['pets_allowed']);
                    $party_allowed          =   sanitize_text_field($_POST['party_allowed']);
                    $children_allowed       =   sanitize_text_field($_POST['children_allowed']);
    ...
    if( isset( $_POST['listing_edit'] ) ) {
        if( !is_numeric($_POST['listing_edit'] ) ){
            ...
            ////////////////////////////////////////////////////////////////////
                ...
                if( !isset($_POST['checkin_message']) ) {
                    $checkin_message='';
                }else{
                    $checkin_message  =   esc_html($_POST['checkin_message']);
                }
                ...
                if($has_errors){
                    ...
                    update_post_meta($post_id, 'checkin_message', $checkin_message);
