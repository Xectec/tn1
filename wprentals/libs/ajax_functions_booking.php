...
if( !function_exists('wpestate_cancel_own_booking') ):
    function wpestate_cancel_own_booking(){
        ...
        // assing booking dates to invoice
        if($invoice_id!=''){
            $booking_from_date  = esc_html(get_post_meta($bookid, 'booking_from_date', true));
            $booking_to_date    = esc_html(get_post_meta($bookid, 'booking_to_date', true));
            update_post_meta($invoice_id,'booking_from_date',$booking_from_date);
            update_post_meta($invoice_id,'booking_to_date',$booking_to_date);
        }
...
if( !function_exists('wpestate_mess_front_end') ):
    function wpestate_mess_front_end(){
        ...
        $booking_guest_no_value = floatval( $_POST['booking_guest_no'] );
        $booking_guest_no.=sprintf( _n('%s Guest', '%s Guests', $booking_guest_no_value, 'wprentals'), number_format_i18n($booking_guest_no_value)  );


        if( isset( $_POST['adults_fvalue']) ){
            $adults_fvalue    =     sanitize_text_field  ( $_POST['adults_fvalue'] );
            $booking_guest_no.=': '.sprintf( _n('%s Adult', '%s Adults', $adults_fvalue, 'wprentals'), number_format_i18n($adults_fvalue)  );
        }

        if( isset( $_POST['childs_fvalue']) ){
            $childs_fvalue   =   sanitize_text_field  ( $_POST['childs_fvalue'] );
            $booking_guest_no.=', '.sprintf( _n('%s Child', '%s Children', $childs_fvalue, 'wprentals'), number_format_i18n($childs_fvalue)  );
        }
        if( isset( $_POST['infants_fvalue']) ){
            $infants_fvalue   =   sanitize_text_field  ( $_POST['infants_fvalue'] );
            $booking_guest_no.=', '.sprintf( _n('%s Infant ', '%s Infants', $infants_fvalue, 'wprentals'), number_format_i18n($infants_fvalue)  );
        }


       

        $booking_from_date  =   sanitize_text_field ( $_POST['booking_from_date']  );
        $booking_to_date    =   sanitize_text_field ( $_POST['booking_to_date']  );

        $contact_u_name     =   '';
        $contact_u_email    =   '';
        
        if( isset( $_POST['contact_u_name']) ){
            $contact_u_name  =   sanitize_text_field ( $_POST['contact_u_name']  );
        }
